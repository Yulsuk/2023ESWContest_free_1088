...
class JudgementTempAndHumControlFunction:
    def __init__(self):
        ...
        self.suitableTempMaxNight = 0.0#set_servo_plusewidth
        self.suitableTempMinNight = 0.0
        self.suitableHumMax = 0.0
        self.suitableHumMin = 0.0
        self.temp = 0.0
        self.hum = 0.0
        self.selectedCorps = 0
    
    def SetSuitableTempAndHum(self, selectedCorps):
        #controlValue = DB.ReadControlValue()
        #controlValue = controlValue[0][0]
        if selectedCorps == '1':#cucumber
        ...
        elif selectedCorps == '2':#pepper
            print("Selected Corps : pepper")
            self.suitableTempMaxDay = 28.0
            self.suitableTempMinDay = 25.0
            self.suitableTempMaxNight = 22.0
            self.suitableTempMinNight = 18.0
            self.suitableHumMax = 90.0
            self.suitableHumMin = 80.0
        elif selectedCorps == '3':#strewberry
            print("Selected Corps : strewberry")
            self.suitableTempMaxDay = 25.0
            self.suitableTempMinDay = 18.0
            self.suitableTempMaxNight = 10.0
            self.suitableTempMinNight = 5.0
            self.suitableHumMax = 70.0
            self.suitableHumMin = 60.0
        elif selectedCorps == '4':#tomato
            print("Selected Corps : tomato")
            self.suitableTempMaxDay = 28.0
            self.suitableTempMinDay = 23.0
            self.suitableTempMaxNight = 12.0
            self.suitableTempMinNight = 8.0
            self.suitableHumMax = 80.0
            self.suitableHumMin = 65.0
...
class ControlInsideWaterSpray:
    ...
    def SetOperateInsideWaterSprayManual(self, manual):
        if manual == 'T':
            GPIO.output(INSIDE_WATERSPRAY,True)
            print('spray is operating')
        else:
...
class ControlGreenHouseSide:
    ...
    def SetOperateGreenHouseSide(self):
        if self.openGreenHouseSide == True and self.closeGreenHouseSide == False and self.closed == True:
            ...
            self.opened = True
            self.closed = False
        elif self.closeGreenHouseSide == True and self.openGreenHouseSide == False and self.opened == True:
            pi.set_servo_pulsewidth(SERVO_MOTOR1,0)
            pi.set_servo_pulsewidth(SERVO_MOTOR2,0)
            time.sleep(1)
    ...
    def SetOperateGreenHouseSideManual(self, manualOpen, manualClose):
        if manualOpen == 'T' and manualClose == 'F' and self.closed == True:
            pi.set_servo_pulsewidth(SERVO_MOTOR1,0)
            pi.set_servo_pulsewidth(SERVO_MOTOR2,0)
            time.sleep(1)
            pi.set_servo_pulsewidth(SERVO_MOTOR1,2500)  
            ...
            self.opened = True
            self.closed = False
        elif manualOpen == 'F' and manualClose == 'T' and self.opened == True:
            pi.set_servo_pulsewidth(SERVO_MOTOR1,0)
            pi.set_servo_pulsewidth(SERVO_MOTOR2,0)
            time.sleep(1)
            pi.set_servo_pulsewidth(SERVO_MOTOR1,500)
...
class ControlVentilator:
    ...
    def SetOperateVentilatorManual(self, manual):
        if manual == 'T':
            GPIO.output(FAN,True)
            print('ventilator is operating')
        else:
...
class ControlHeatingWire:
    ...
    def SetOperateHeatingWire(self):
        if self.operateHeatingWire == True:
            GPIO.output(HEATINGWIRE,True)
            print('heatingWire is operating')
        else:
            GPIO.output(HEATINGWIRE,False)
            print('heatingWire is idle')
        
    def SetOperateHeatingWireManual(self, manual):
        if manual == 'T':
            GPIO.output(HEATINGWIRE,True)
            print('heatingWire is operating')
        else:
